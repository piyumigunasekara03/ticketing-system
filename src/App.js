...
function App() {
  ...
  const handleStart = () => {
    ...
    setIsRunning(true);
    logMessage(
      `Quantum Ticket Exchange initialized. Total Tickets: ${config.totalTickets}, Release Rate: ${config.ticketReleaseRate}/min, Retrieval Rate: ${config.customerRetrievalRate}/min, Vendors: ${config.vendorCount}`
    );
  };
